...
function getWishlist(){
    try{
        const value = JSON.parse(localStorage.getItem("freedelivery_wishlist") || "[]");
        return Array.isArray(value) ? value : [];
    }catch(e){ return []; }
}

function saveWishlist(items){
    localStorage.setItem("freedelivery_wishlist", JSON.stringify(items));
}
...
function toggleWishlist(button, product){
    ...
    if(index >= 0){
        ...
        items.push({id, name: product.name, price: product.price, image: product.image, category: product.category});
        button.classList.add("is-wishlisted");
    }
    saveWishlist(items);
    button.querySelector("i")?.classList.toggle("fa-solid", index < 0);
    button.querySelector("i")?.classList.toggle("fa-regular", index >= 0);
}

function syncWishlistButtons(){
    const ids = new Set(getWishlist().map(item => Number(item.id)));
    document.querySelectorAll(".wishlist[data-product-id]").forEach(button => {
...
function searchProducts(){
    ...
    document.querySelectorAll(".product-card").forEach(card => {
        ...
        const cardCategory = (card.querySelector(".category-name")?.textContent || "").toLowerCase();
        card.hidden = (query && !name.includes(query)) || (category && cardCategory !== category);
    });
}
...
function openWishlist(){
    const items = getWishlist();
    openModal(`
        <h2><i class="fa-solid fa-heart"></i> আমার Wishlist</h2>
        ${items.length ? items.map(item => `
            <article class="wishlist-row">
                <img src="${escapeAccountText(item.image || "https://placehold.co/80x80?text=Product")}" alt="">
                <div><strong>${escapeAccountText(item.name)}</strong><small>৳ ${Number(item.price).toLocaleString("en-US")}</small></div>
                <button type="button" onclick="removeWishlist(${Number(item.id)})"><i class="fa-solid fa-trash"></i></button>
            </article>`).join("") : '<p class="empty-state">Wishlist এখনো খালি।</p>'}
    `);
}
...
if(customerOrdersRefreshTimer){
    clearInterval(customerOrdersRefreshTimer);
    customerOrdersRefreshTimer = null;
}
...
function showPromoOffer(){
    ...
    openModal(`
        ...
        <div style="text-align:center;">
            ...
            <div style="font-size:52px;margin-bottom:10px; color:#16a34a;"><i class="fa-solid fa-circle-check"></i></div>
            ...
            <button
                ...
                onclick="closeModal()">
                <i class="fa-solid fa-bag-shopping"></i> কেনাকাটা করুন
            </button>
...
openModal(`  
    ...
    <h2><i class="fa-solid fa-bullseye"></i> আমাদের ভিশন ও মিশন</h2>  
...
async function openAccount(){
    ...
    openModal(`
        <div style="text-align:center;">
            ...
            "><img id="accountProfileImage" src="" alt="Profile" style="width:100%;height:100%;object-fit:cover;border-radius:50%;display:none;"><i id="accountProfileIcon" class="fa-solid fa-user"></i></div>

            <input id="profilePhotoInput" type="file" accept="image/jpeg,image/png,image/webp" hidden onchange="uploadProfilePhoto(this)">
            <button type="button" class="modal-btn" style="width:auto;margin:0 auto 12px;padding:8px 14px;" onclick="document.getElementById('profilePhotoInput').click()"><i class="fa-solid fa-camera"></i> ছবি আপলোড / পরিবর্তন</button>
    ...
    try{
        ...
        try{
            ...
            console.error("Order API raw response:", responseText);
            throw new Error(
                responseText.trim()
                || "সার্ভার থেকে কোনো response পাওয়া যায়নি। Production database/configuration চেক করুন।"
            );
        }
    ...
    const promoHtml = promo
        ...
        : `
            <div style="
                ...
                font-size:14px;
            ">
                <i class="fa-solid fa-gift"></i> এখনো কোনো Free Delivery Code নেই।
                <br>
    ...
    box.innerHTML=`
        ...
        ">
            ...
            <div style="
                ...
                color:#4b5563;
            ">
                <i class="fa-solid fa-phone"></i> ${escapeAccountText(customer.phone || "")}
            </div>
            ...
            ${
                ...
                ? `<div style="
                    ...
                    font-size:13px;
                    color:#6b7280;
                "><i class="fa-solid fa-location-dot"></i> ${escapeAccountText(customer.address)}</div>`
                : ""
            }
        </div>
        ...
        <div style="margin-top:17px;text-align:left;">
            <h3 style="
                ...
                font-size:16px;
            ">
                <i class="fa-solid fa-gift"></i> আমার Free Delivery
            </h3>
...
async function uploadProfilePhoto(input){
    ...
    if(!file) return;
    if(file.size > 5 * 1024 * 1024){ showNotification("প্রোফাইল ছবির আকার ৫MB-এর মধ্যে হতে হবে।", "error"); return; }
    const saved = JSON.parse(localStorage.getItem("customer_session") || "null");
    if(!saved?.phone){ showNotification("আগে লগইন করুন।", "warning"); return; }
...
async function loadCustomerOrders(phone){
    ...
    try{
        ...
        `).join('');
        customerOrdersRefreshTimer = setInterval(() => loadCustomerOrders(phone), 15000);
    }catch(error){
...
window.addToCart = function addToCart(productId, name, price, image){
    if(!productId || !name || !Number.isFinite(Number(price))){
        showNotification("এই পণ্যটি কার্টে যোগ করা যাচ্ছে না। তথ্য অসম্পূর্ণ।", "error");
        return;
    }
    ...
    showNotification("পণ্যটি সফলভাবে কার্টে যোগ হয়েছে।", "success");
}
...
window.buyNow = function buyNow(productId, name, price, image){
    ...
    if(!productId || !name || !Number.isFinite(Number(price))){
        showNotification("অর্ডার শুরু করা যাচ্ছে না। পণ্যের তথ্য অসম্পূর্ণ।", "error");
        return;
    }
    ...
    showCartCheckout();
    showNotification("অর্ডার ফর্ম প্রস্তুত হয়েছে। আপনার তথ্য দিয়ে অর্ডার সম্পন্ন করুন।", "success");
}
    ...
    openModal(`
        ...
        <div class="professional-checkout">
            ...
            ">
                ...
                <strong>
                    <i class="fa-solid fa-truck-fast"></i> ডেলিভারি চার্জ
                </strong>
            ...
            <div class="checkout-payment">
                ...
                <strong>
                    <i class="fa-solid fa-credit-card"></i> পেমেন্ট
                </strong>
...
async function applyCheckoutPromo(){
    ...
    if(promoCode.length < 8 || promoCode.length > 32){
        showNotification("প্রোমো কোডটি ৮ থেকে ৩২ অক্ষরের হতে হবে। আপনার account-এর পুরনো ১৪-character code-ও ব্যবহার করা যাবে।", "error");
        promoInput?.focus();
        return;
    }
                ...
                ${data.promo_code ? `
                    ...
                    ">
                        <div style="
                            ...
                            line-height:1.6;
                        ">
                            <i class="fa-solid fa-gift"></i> আপনি ১ বছরের Free Delivery Promo Code পেয়েছেন
                        </div>
                        ...
                        ">
                            <button
                                ...
                                <i class="fa-regular fa-copy"></i> কোড কপি করুন
                            </button>
                            ...
                            <button
                                ...
                                <i class="fa-solid fa-floppy-disk"></i> কোড সেভ করুন
                            </button>
                        </div>
                    </div>
                ` : ""}
