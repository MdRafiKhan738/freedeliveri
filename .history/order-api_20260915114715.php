...
function findValidPromo(PDO $pdo, string $phone, string $promoCode, float $subtotal): ?array
{
    ...
    $sql = "
        ...
        WHERE u.phone = ?
          ...
          AND (pc.usage_limit IS NULL OR pc.usage_limit = 0 OR pc.used_count < pc.usage_limit)
          AND (up.usage_count = 0)
...
if ($action === "validate_promo") {
    ...
    try {
        if (strlen($promo) !== 8) {
            jsonResponse(false, "প্রোমো কোড ৮ অক্ষরের হতে হবে। উদাহরণ: FD260915", [], 400);
        }
...
if ($action === "account") {
    ...
    try {
        $stmt = $pdo->prepare("SELECT id, name, phone FROM users WHERE phone = ? AND role = 'customer' LIMIT 1");
        $stmt->execute([$phone]);
        $customer = $stmt->fetch();
