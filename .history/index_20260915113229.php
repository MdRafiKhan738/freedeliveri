...
function productImage(?string $path): string {
    ...
    // Actual product image folder:
    // public_html/admin/uploads/products/
...
function renderProductCard(array $product): void {
    ...
    <article class="product-card" onclick='openProductDetails(<?= $productDetailsJson ?>)'>
        <div class="product-image">
            ...
            <button class="wishlist" type="button" aria-label="Wishlist" onclick="event.stopPropagation()">
    ...
    modalContent.innerHTML = `
        <div class="product-details">
            <div class="product-details-gallery">
                <button type="button" class="gallery-arrow" onclick="changeProductMedia(-1)"><i class="fa-solid fa-chevron-left"></i></button>
                <div class="product-details-media-stage" id="productDetailsMediaStage">${media.map((item, index) => item.type === "video"
                    ? `<video class="product-details-media ${index === 0 ? "active" : ""}" src="${safe(item.url)}" controls playsinline></video>`
                    : `<img class="product-details-media ${index === 0 ? "active" : ""}" src="${safe(item.url)}" alt="${safeName}">`).join("")}</div>
                <button type="button" class="gallery-arrow" onclick="changeProductMedia(1)"><i class="fa-solid fa-chevron-right"></i></button>
            </div>
            <div class="product-media-thumbs">${media.map((item, index) => `<button type="button" onclick="showProductMedia(${index})" aria-label="View media ${index + 1}">${index + 1}</button>`).join("")}</div>
