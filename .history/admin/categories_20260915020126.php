...
<input type="hidden" name="action" value="<?= $editing ? 'update' : 'create' ?>"><input type="hidden" name="id" value="<?= (int)($editing['id'] ?? 0) ?>">
...
<?php foreach ($categories as $category): ?><tr><td><?php if ($category['image_mime']): ?><img class="thumb" src="../image-api.php?type=category&id=<?= (int)$category['id'] ?>" alt="<?= catEscape($category['name']) ?>"><?php else: ?>No image<?php endif; ?></td><td><strong><?= catEscape($category['name']) ?></strong></td><td><?= catEscape($category['created_at']) ?></td><td><a class="action" href="categories.php?edit=<?= (int)$category['id'] ?>">Edit</a><a class="action danger" href="categories.php?delete=<?= (int)$category['id'] ?>" onclick="return confirm('Delete this category?')">Delete</a></td></tr><?php endforeach; ?>
</tbody></table></section>
</main></body></html>
<?php
declare(strict_types=1);
