...
if (isset($_FILES["product_media"]) && is_array($_FILES["product_media"])) {

    $uploadResult = uploadProductMediaToMysql($_FILES["product_media"]);
    if (!$uploadResult["success"]) {
        $error = $uploadResult["message"];
    } else {
        $productMedia = $uploadResult["items"] ?? [];
        foreach ($productMedia as $media) {
            if (($media["media_type"] ?? "") === "image") {
                $imageData = $media["data"] ?? null;
                $imageMime = $media["mime"] ?? null;
                $imageUrl = "image-api.php?type=product&id=" . $productId;
                break;
            }
        }
    }
...
try {
            ...
            if ($productMedia !== []) {
                $mediaStmt = $pdo->prepare("INSERT INTO product_media (product_id, media_type, media_data, media_mime, sort_order) SELECT ?, ?, ?, ?, COALESCE(MAX(sort_order), -1) + 1 FROM product_media WHERE product_id = ?");
                foreach ($productMedia as $media) {
                    $mediaStmt->execute([
                        $productId,
                        $media["media_type"],
...
>
    ...
    <div class="form-group">
        ...
        <div class="small-note">
            Add multiple images/videos. Images up to 5MB, videos up to 50MB.
        </div>
