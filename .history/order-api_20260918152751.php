...
register_shutdown_function(static function (): void {
    ...
    echo json_encode([
        'success' => false,
        'message' => 'Production server error: ' . $error['message'],
...
if ($action === "profile_photo") {
    ...
    try {
        if (@getimagesizefromstring($binary) === false) {
            jsonResponse(false, "প্রোফাইল ছবিটি সঠিক নয়।", [], 400);
        }
...
if ($action === "account") {
    ...
    try {
        $stmt = $pdo->prepare("SELECT id, name, phone, CASE WHEN profile_image_data IS NOT NULL AND profile_image_mime IS NOT NULL THEN CONCAT('image-api.php?type=profile&phone=', phone) ELSE NULL END AS photo_url FROM users WHERE phone = ? AND role = 'customer' LIMIT 1");
        $stmt->execute([$phone]);
        $customer = $stmt->fetch();
