            ...
            <div class="product-media-stage">
                <?php foreach ($media as $mediaIndex => $mediaItem): ?>
                    <?php if (($mediaItem['type'] ?? '') === 'video'): ?>
                        <video class="<?= $mediaIndex === 0 ? 'active' : '' ?>" src="<?= e($mediaItem['url']) ?>" muted loop playsinline preload="metadata" data-media-index="<?= $mediaIndex ?>"></video>
                    <?php else: ?>
                        <img class="<?= $mediaIndex === 0 ? 'active' : '' ?>" src="<?= e($mediaItem['url']) ?>" alt="<?= e($name) ?>" loading="lazy" data-media-index="<?= $mediaIndex ?>">
                    <?php endif; ?>
                <?php endforeach; ?>
            </div>
        </div>
...
.product-details-gallery{display:flex;align-items:center;gap:8px;margin-bottom:10px}
.product-details-media-stage{flex:1;min-width:0;height:min(52vw,360px);overflow:hidden;border-radius:14px;background:#f8fafc;display:flex;align-items:center;justify-content:center}
.product-details-media{display:none;width:100%;height:100%;object-fit:contain;cursor:zoom-in;transition:transform .25s ease}
.product-details-media.active{display:block}
.product-details-media.is-zoomed{transform:scale(1.8);cursor:zoom-out}
.product-details-media-stage video{background:#111}
.gallery-arrow{width:36px;height:36px;flex:0 0 36px;border-radius:50%;background:#111827;color:#fff}
.product-media-thumbs{display:flex;gap:7px;overflow-x:auto;margin-bottom:14px}
.product-media-thumbs button{min-width:34px;height:34px;border-radius:8px;background:#f3f4f6;color:#111827}
.product-media-thumbs button.active{background:#16a34a;color:#fff}
...
<header class="header">
    ...
    <div class="top-header">
        ...
        <div class="header-icons">
            ...
            <button id="headerAccountButton" onclick="openAccount()" aria-label="Account">
                <i id="headerAccountIcon" class="fa-regular fa-user"></i>
            </button>
        </div>
...
<aside class="side-menu" id="sideMenu">
    ...
    <div class="menu-list">
        ...
        <a href="wishlist.php" onclick="closeMenu()">
...
<section class="social-visit" aria-label="Social contact">
    <a href="https://www.facebook.com/profile.php?id=61593239572630" target="_blank" rel="noopener">
...
<footer class="footer" id="about">
    ...
    <div class="design-credit">
        Design and Developed by <a href="https://mdrafikhan.vercel.app" target="_blank" rel="noopener">Mohammad Rafi Khan</a>
    </div>
...
function openProductDetails(product) {
    ...
    window.activeDetailProduct = {id: Number(product.id) || 0, name, price, image: String(media[0]?.url || product.image || "")};
        ...
        <div class="product-details">
            <div class="product-details-gallery">
                ...
                <div class="product-details-media-stage" id="productDetailsMediaStage">${media.map((item, index) => item.type === "video"
                    ...
                    : `<img class="product-details-media zoomable ${index === 0 ? "active" : ""}" src="${safe(item.url)}" alt="${safeName}" loading="eager" onclick="toggleImageZoom(this)">`).join("")}</div>
            ...
            <button type="button" class="product-details-order"
                    onclick="event.stopPropagation(); buyCurrentDetailProduct()">
                    <i data-lucide="shopping-bag"></i>
                অর্ডার করুন
            </button>

            <button type="button" class="product-details-cart"
                    onclick="event.stopPropagation(); addCurrentDetailProduct()">
...
function buyCurrentDetailProduct(){
    const product = window.activeDetailProduct;
    if(!product){
        showNotification("অর্ডার শুরু করা যাচ্ছে না। পণ্যটি আবার নির্বাচন করুন।", "error");
        return;
    }
    buyNow(product.id, product.name, product.price, product.image);
}

function addCurrentDetailProduct(){
    const product = window.activeDetailProduct;
    if(!product){
        showNotification("পণ্যটি কার্টে যোগ করা যাচ্ছে না। পণ্যটি আবার নির্বাচন করুন।", "error");
        return;
    }
