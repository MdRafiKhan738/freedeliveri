...
<section class="section" id="categories">
    ...
    <div class="category-grid">
        <?php if (!empty($categories)): ?>
            <?php
            // Category icons follow the fixed category order.
            $categoryIcons = ['🔫','🍳','🎮','🎧','💄','👗','👔'];
            foreach ($categories as $i => $category):
            ?>
                <a class="category-card" href="?category=<?= (int)$category['id'] ?>#products">
                    <div class="category-icon"><?= e($categoryIcons[$i % count($categoryIcons)]) ?></div>
...
function openProductDetails(product) {
    ...
    const image = String(product.image || "https://placehold.co/600x600?text=Product");
    ...
    modalContent.innerHTML = `
        <div class="product-details">
            ...
            <button type="button" class="product-details-order"
                    onclick="buyNow(${Number(product.id) || 0}, ${JSON.stringify(name)}, ${JSON.stringify(price)}, ${JSON.stringify(image)})">
                <i class="fa-solid fa-bag-shopping"></i>
                অর্ডার করুন
            </button>

            <button type="button" class="product-details-cart"
                    onclick="addToCart(${Number(product.id) || 0}, ${JSON.stringify(name)}, ${JSON.stringify(price)}, ${JSON.stringify(image)})">
                <i class="fa-solid fa-cart-plus"></i>
                কার্টে যোগ করুন
            </button>
